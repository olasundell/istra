...
description: Report concrete or strongly suspected faults in Istra's MCP tools, Hermes plugin, bundled instructions, or prescribed workflow. Use when Istra appears to contradict its contract, lose or corrupt state, package incorrectly, or present a materially unsafe, contradictory, or repeatedly misleading design.
---

# Istra Error Reporting

Use `mcp__istra__report_error` to record Istra faults autonomously without interrupting the user's task.

Use `client: "hermes-plugin:istra"` on every report.
...
3. Provide a concise component, summary, and observation. Keep observations separate from inferences. Add expected and actual behaviour, minimal reproduction steps, impact, and resolved project or workspace context only when useful.
...
- Report only Istra faults: its MCP server, Hermes plugin packaging, bundled instructions, or workflow.
...
- Never report a `mcp__istra__report_error` failure through `mcp__istra__report_error`, retry it recursively, or fall back to work items, updates, REST, or direct SQLite access. Mention the unavailable reporter once if it materially affects the task, then continue.
