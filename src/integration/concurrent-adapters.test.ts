...
describe('concurrent HTTP and MCP writes', () => {
  ...
  it('replays one stored result across two connections and rejects mismatched key reuse', async () => {
    ...
    await Promise.all([httpRuntime.backupManager.beforeWrite(), mcpRuntime.backupManager.beforeWrite()])
    ...
    expect(httpRuntime.service.listRequirements(project.id).filter(({ stableKey }) => stableKey === 'RACE-1')).toHaveLength(1)
    expect(httpRuntime.service.listActivity(project.id).filter(({ eventType, entityId }) => eventType === 'requirement.created' && entityId === first.id)).toHaveLength(1)
  ...
  it('rolls an interrupted writer transaction back so an idempotent retry can complete', async () => {
    ...
    const state = httpRuntime.operationalRepository.listRequirementStates(project.id)[0]!
    const worker = new Worker(`
      ...
      process.exit(0);
    `, { eval: true, workerData: { databasePath: httpRuntime.paths.databasePath, projectId: project.id, stateId: state.id } })
    await new Promise<void>((resolvePromise, rejectPromise) => {
      worker.once('exit', (code) => code === 0 ? resolvePromise() : rejectPromise(new Error(`worker exited ${code}`)))
      worker.once('error', rejectPromise)
    })

    expect(httpRuntime.service.listRequirements(project.id).some(({ stableKey }) => stableKey === 'CRASH-1')).toBe(false)
    ...
    expect(httpRuntime.service.listRequirements(project.id).filter(({ stableKey }) => stableKey === 'CRASH-1')).toHaveLength(1)
  })
})
